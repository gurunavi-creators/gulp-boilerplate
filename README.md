...
[Pug](https://github.com/pugjs/pug)（旧 Jade） から HTML へ出力します。
...
- `_`（アンダースコア）から始まるディレクトリの中にある js ファイルを、ファイル名の昇順ソート順に連結し、ディレクトリ名から`_`を除いたファイル名で出力します。
- `js` タスクでは [Browserfy](http://browserify.org/) を使って連結するため、グローバル変数の汚染がありませんが、読み込んだ JS を単純にグローバルに登録したい場合は こちらのタスクの方が向いています。
